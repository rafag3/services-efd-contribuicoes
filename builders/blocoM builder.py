...
def build_blocoM(notas: list[NotaFiscal], f600s: list[RegistroF600] = None,
    ...
    # VL_RET_NC = soma F600 VL_RET_PIS (retenções do período)
    ret_nc_pis = round(sum(f.vl_ret_pis for f in f600s), 2) if f600s else 0.0

    # VL_CONT_NC_REC = total devida - retido na fonte (saldo a recolher)
    ...
    linhas.append(montar_linha(
        ...
        fmt_valor(ret_nc_pis),       # 05 VL_RET_NC = soma F600
        "0",                          # 06 VL_OUT_DED_NC
        fmt_valor(cont_nc_rec_pis),  # 07 VL_CONT_NC_REC = DEV - RET
        "0",                          # 08 VL_TOT_CONT_CUM_PER
        "0",                          # 09 VL_RET_CUM
        "0",                          # 10 VL_OUT_DED_CUM
        "0",                          # 11 VL_CONT_CUM_REC
        fmt_valor(cont_nc_rec_pis),  # 12 VL_TOT_CONT_REC = NC + CUM
    ))
    ...
    linhas.append(montar_linha(
        ...
        fmt_valor(ret_nc_cofins),        # 05 VL_RET_NC = soma F600
        "0",                              # 06 VL_OUT_DED_NC
        fmt_valor(cont_nc_rec_cofins),   # 07 VL_CONT_NC_REC = DEV - RET
        "0",                              # 08 VL_TOT_CONT_CUM_PER
        "0",                              # 09 VL_RET_CUM
        "0",                              # 10 VL_OUT_DED_CUM
        "0",                              # 11 VL_CONT_CUM_REC
        fmt_valor(cont_nc_rec_cofins),   # 12 VL_TOT_CONT_REC = NC + CUM
    ))
